...
{/if}
		...
		<![endif]-->
		<link rel="stylesheet" href="{$css_dir}SA/SA_pure.css" type="text/css" media="all" />
		<link rel="stylesheet" href="{$css_dir}SA/SA_theme.css" type="text/css" media="all" />
		<script type="text/javascript" src="{$js_dir}wow.js"></script>
		<script>
 			new WOW().init();
		</script>
							...
							<div class="row">
								<div id="header_logo">
									<a href="{if $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}" title="{$shop_name|escape:'html':'UTF-8'}">
										<img class="logo img-responsive" src="{$logo_url}" alt="Oeil de sainte lucie {$shop_name|escape:'html':'UTF-8'}" title="Spécialiste de l'oeil de sainte Lucie"{if isset($logo_image_width) && $logo_image_width} width="{$logo_image_width}"{/if}{if isset($logo_image_height) && $logo_image_height} height="{$logo_image_height}"{/if}/>
									</a>
								</div>
